...
# Read global outputs from the global remote state file from global root  (centralized cross-stack wiring)
...
module "IAM_lambda_regional" {
  source                       = "../../../modules/security/IAM_lambda_regional"
  sqs_fifo_arn                 = module.sqs_fifo.queue_arn
  sqs_fifo_arn_dlq             = module.sqs_fifo.queue_arn_dlq
  sqs_fifo_name                = module.sqs_fifo.queue_name
  sqs_fifo_eventbridge_arn_dlq = module.sqs_fifo.queue_event_bridge_arn_dlq
  dynamodb_idemp_arn           = module.dynamodb-table.table_arn
  lambda_exec_role_name        = data.terraform_remote_state.prod_global.outputs.lambda_exec_role_name
}
